...
class TestBasicUsage:
    ...
    def test_basic_usage_from_connection_to_disconnection_with_crud_process(
        ...
        with self.runner.isolated_filesystem():
            ...
            new_commercial = mocked_session.scalar(
                select(User).where(User.email == commercial_email)
            )
            ...
            new_support = mocked_session.scalar(
            ...
            new_contract = mocked_session.scalar(
                select(Contract).where(Contract.client_id == new_client.id)
            )
            ...
            assert all(
                ...
                for arg in [str(new_contract.id), str(left_to_pay), status]
            )
            ...
            new_event = mocked_session.scalar(
                select(Event).where(Event.contract_id == new_contract.id)
            )
            ...
            assert result.exit_code == 0
            assert (
                f"{new_support.name} is now responsible for the event {new_event.id}"
                in result.output
            )
            ...
            assert result.exit_code == 0
            assert all(
                arg in result.output for arg in [str(new_event.id), str(attendees)]
            )
            ...
            deleted_client = mocked_session.scalar(
                select(Client).where(Client.id == new_client.id)
            )
            old_contract = mocked_session.scalar(
                select(Contract).where(Contract.id == new_contract.id)
            )
            old_event = mocked_session.scalar(
                select(Event).where(Event.id == new_event.id)
            )
