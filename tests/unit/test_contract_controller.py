...
from epic_events.controllers.contract_controller import (check_amount,
                                                         create_contract,
                                                         delete_contract,
                                                         get_contract,
                                                         list_contracts,
                                                         update_contract)
from epic_events.models import Contract, User


@pytest.mark.parametrize("filter_option, expected_count", [(None, 2), ("-s SIGNED", 1)])
...
class TestUpdateContractController:
    ...
    @pytest.mark.parametrize(
        "contract_id, amount, left_to_pay, status", [(2, 90, 10, "SIGNED")]
    )
    def test_update_contract_with_correct_argument(
        self, mocked_session, contract_id, amount, left_to_pay, status
    ):
        current_user = mocked_session.scalar(select(User).where(User.id == 3))
        updated_contract = mocked_session.scalar(
        ...
        assert result.exit_code == 0
        assert all(
            arg in result.output
            for arg in [str(contract_id), str(amount), str(left_to_pay), status]
        )
...
class TestGetContractController:
    ...
    @pytest.mark.parametrize("contract_id", [1])
...
class TestCreateContractController:
    ...
    @pytest.mark.parametrize("client_id, amount", [(777, 100)])
    def test_create_contract_with_unknown_client(
        self, mocked_session, client_id, amount
    ):
        current_user = mocked_session.scalar(select(User).where(User.id == 1))
    ...
    @pytest.mark.parametrize("client_id, amount", [(1, 100)])
    def test_create_contract_with_correct_argument(
        self, mocked_session, client_id, amount
    ):
        current_user = mocked_session.scalar(select(User).where(User.id == 1))
        options = ["-client", client_id, "-a", amount]
...
class TestCheckAmount:
    @pytest.mark.parametrize("amount, left_to_pay", [(100, 50)])
    def test_check_amount_with_correct_argument(self, amount, left_to_pay):
        result = check_amount(amount, left_to_pay)
        assert result is True

    @pytest.mark.parametrize("amount, left_to_pay", [(100, 200)])
    def test_check_amount_with_left_to_pay_bigger_than_amount(
        self, amount, left_to_pay
    ):
        error = (
