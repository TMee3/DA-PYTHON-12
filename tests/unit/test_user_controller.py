...
class TestCreateUserController:
    ...
    @pytest.mark.parametrize("role, expected_exit_code, expected_output", [
        ("777", 1, "777 is not a correct role."),
        ("1", 0, "User leo@test.com is successfully created")
    ])
    def test_create_user_with_unknown_role(self, mocked_session, role, expected_exit_code, expected_output):
    ...
    @pytest.mark.parametrize("name, email, role, expected_exit_code, expected_output", [
        ("lucas", "manager_lucas@test.com", "3", 1, "User (manager_lucas@test.com) already exists."),
        ("leo", "leo@test.com", "1", 0, "User leo@test.com is successfully created")
    ])
    def test_create_user_with_existing_user(self, mocked_session, name, email, role, expected_exit_code, expected_output):
        current_user = mocked_session.scalar(select(User).where(User.id == 1))
        options = ["-n", name, "-e", email, "-r", role, "--password", "1234"]
...
class TestGetUserController:
    ...
    @pytest.mark.parametrize("id_argument, expected_exit_code, expected_output", [
        (None, 2, "Missing option"),
        ("777", 1, "Sorry, this user does not exist."),
        ("ab", 2, "Invalid value"),
        ("2", 0, "User(id=2")
    ])
    def test_get_user(self, mocked_session, id_argument, expected_exit_code, expected_output):
        current_user = mocked_session.scalar(select(User).where(User.id == 1))
        options = []
        if id_argument:
...
class TestUpdateUserController:
    ...
    @pytest.mark.parametrize("id_argument, name_argument, email_argument, role_argument, expected_exit_code, expected_output", [
        (2, "leo", "commercial_leo@test.com", 1, 0, "User commercial_leo@test.com is successfully updated"),
        (2, None, None, None, 1, "Can't update without data in the command."),
        (2, "leo", None, 777, 1, "777 is not a correct role.")
    ])
    def test_update_user(self, mocked_session, id_argument, name_argument, email_argument, role_argument, expected_exit_code, expected_output):
        current_user = mocked_session.scalar(select(User).where(User.id == 1))
...
class TestDeleteUserController:
    ...
    @pytest.mark.parametrize("user_id, confirmation_input, expected_exit_code, expected_output", [
        (777, "y", 1, "Sorry, this user does not exist."),
        (3, "y", 0, "This user is successfully deleted"),
        (3, "N", 1, "Aborted")
    ])
    def test_delete_user(self, mocked_session, user_id, confirmation_input, expected_exit_code, expected_output):
        current_user = mocked_session.scalar(select(User).where(User.id == 1))
        options = ["-id", str(user_id)]
        result = self.runner.invoke(delete_user, options, input=confirmation_input, obj={"session": mocked_session,
                                                                                       "current_user": current_user})
        assert result.exit_code == expected_exit_code
        assert expected_output in result.output
