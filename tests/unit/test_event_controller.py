...
class TestListEventController:
    ...
    def test_list_events(self, mocked_session, options, expected_count):
        ...
        result = self.runner.invoke(list_events, options, obj={"session": mocked_session, "current_user": current_user})
...
class TestCreateEventController:
    ...
    @pytest.mark.parametrize("contract_id, current_user_id, expected_exit_code, expected_output", [
        (777, 3, 1, "Sorry, this contract does not exist."),
        (1, 4, 1, "Sorry, you're not authorized."),
        (2, 3, 1, "Sorry, the contract (id:2) is not signed, we can't create an event."),
        (1, 3, 0, "Event 2 is successfully created.")
    ])
    def test_create_event(self, mocked_session, contract_id, current_user_id, expected_exit_code, expected_output):
...
class TestUpdateEventSupportContactController:
    ...
    @pytest.mark.parametrize("event_id, support_contact_id, current_user_id, expected_exit_code, expected_output", [
    ...
    def test_update_event_support_contact(self, mocked_session, event_id, support_contact_id, current_user_id, expected_exit_code, expected_output):
        current_user = mocked_session.scalar(select(User).where(User.id == current_user_id))
...
class TestGetEventController:
    ...
    @pytest.mark.parametrize("event_id, current_user_id, expected_exit_code, expected_output", [
        (777, 2, 1, "Sorry, this event does not exist."),
        (1, 2, 0, "Id: 1")  # Ajoutez d'autres cas de test si nécessaire
    ])
    def test_get_event(self, mocked_session, event_id, current_user_id, expected_exit_code, expected_output):
        current_user = mocked_session.scalar(select(User).where(User.id == current_user_id))
...
class TestDeleteEventController:
    ...
    @pytest.mark.parametrize("event_id, current_user_id, input_response, expected_exit_code, expected_output", [
        (777, 2, "y", 1, "Sorry, this event does not exist."),
        (1, 5, "y", 1, "Sorry, you're not authorized."),
        (1, 2, "y", 0, "This event is successfully deleted.")
    ])
    def test_delete_event(self, mocked_session, event_id, current_user_id, input_response, expected_exit_code, expected_output):
        current_user = mocked_session.scalar(select(User).where(User.id == current_user_id))
        options = ["-id", str(event_id)]
        result = self.runner.invoke(delete_event, options, input=input_response, obj={"session": mocked_session, "current_user": current_user})
        assert result.exit_code == expected_exit_code
        assert expected_output in result.output
...
class TestCheckDate:
    ...
    @pytest.mark.parametrize("start_date, end_date, expected_exception_message", [
        (datetime.now() + timedelta(days=1), datetime.now() + timedelta(days=2), None),
        (datetime.now() + timedelta(days=2), datetime.now() + timedelta(days=1), "End date .* can't be before start date .*."),
        (datetime.now() - timedelta(days=2), datetime.now() - timedelta(days=1), "Start date and end date cannot be in the past")
    ])
